...
import { X, Star, GripVertical, Check, Plus, Trash2, Upload, Folder, RotateCcw } from 'lucide-react'
...
function DynamicProjectPanel({ project, onDelete }) {
      ...
      <div className="flex flex-1 overflow-hidden">
          ...
          {PROJECTS.map(p => {
            ...
            return (
              <button key={p.id}
                ...
                <div className="w-10 h-10 flex-shrink-0 overflow-hidden bg-dark-grey">
                  <img src={coverImg} alt="" className="w-full h-full object-cover"
                    draggable={false} onContextMenu={e => e.preventDefault()} />
                </div>
                <div className="min-w-0">
                  <p className={`text-[11px] leading-tight truncate ${activeProject === p.id && mode === 'manage' ? 'text-gold' : 'text-white/70'}`}>{p.title}</p>
                  <p className="text-white/30 text-[9px] mt-0.5">{p.images.length} images</p>
                </div>
              </button>
            )
          })}
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {mode === 'manage' && isViewingDynamic && dynamicProject && (
            <DynamicProjectPanel project={dynamicProject} onDelete={() => handleDeleteDynamic(dynamicProject.id)} />
          )}
          ...
          {mode === 'manage' && !isViewingDynamic && staticProject && (
            <div className="p-6">
              <div className="mb-4 flex items-center justify-between">
                <h2 className="text-white/80 text-sm font-medium">{staticProject.title}</h2>
                <span className="text-white/30 text-[10px]">{orderedImages.length} images · drag to reorder · ★ to set cover</span>
              </div>
              <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
                {orderedImages.map((img, displayIdx) => {
                  const origIdx    = staticProject.images.indexOf(img)
                  const isCover    = origIdx === coverOrigIdx
                  const isDragTarget = dragOver === displayIdx
                  return (
                    <div key={`${staticProject.id}-${origIdx}`} draggable
                    ...
                    >
                      ...
                      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-all duration-200 flex items-center justify-center">
                        <button onClick={() => setCover(staticProject.id, origIdx)}
